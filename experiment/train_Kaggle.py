...
from dataProcess.Kaggle_process_k_flod import cross_validation, KaggleDataset, targets
...
# 使用该注意力模块的模型
class FusionKaggle(nn.Module):
    def __init__(self, in_channels, num_classes=2):
        super(FusionKaggle, self).__init__()
        ...
        # 全连接层
        self.fc1 = nn.Linear(128 * 7 * 12, 256)
...
if __name__ == '__main__':
    ...
    for indexPat in range(0, len(dataset)):
        ...
        for k_fold, (train_index, val_index) in enumerate(kf.split(patients_datas)):
            ...
            train_dataset = KaggleDataset(train_data, train_labels)
            val_dataset = KaggleDataset(val_data, val_labels)
            batch_size = 64
            ...
            path = f"/e/wht_project/new_eeg_data/k_fold10/Kaggle"
            os.makedirs(path, exist_ok=True)

            # 创建模型
            model = FusionKaggle(in_channels=16)
            model.to(device)
